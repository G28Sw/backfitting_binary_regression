...
obj<-function(x,z,w,beta,a,b,lambda_a,lambda_b,f1,f2){
  ###returns the penalized least square objective value
  return(sum(w*(z-x%*%beta-a[f1]-b[f2])^2)+lambda_a*sum(a^2)+lambda_b*sum(b^2))}
expit<-function(t){
  return(1/(1+exp(-t)))
}

backfitting_inner_loop<-function(x,z,f1,f2,weight,weight_a,weight_b,beta_hat,a_hat,b_hat,
                                        lambda_a_hat,lambda_b_hat,max_iter=100,epsilon=1e-10,
                                        inner_trace.it,Xclub_a,Xclub_b){
  ...
  while((relative_change > epsilon)&(iter<=max_iter)){ 
    ...
    obj_clubbing = obj(x,z,weight,beta_hat,a_hat,b_hat,lambda_a_hat,lambda_b_hat,f1,f2)
    cat("Iter : ",iter,"objective function after modifying both a and b is : ",obj_clubbing,"\n")
    
    
    if(inner_trace.it){cat("relative_change inner loop : ",relative_change,"\n")}
  }
    ...
    cat("estimate of sigma_a_sq : ",sigma_sq_a_hat,"\n")
...
backfitting_outer_loop<-function(x,y,f1,f2,betahat = rep(0,ncol(x)),sigma_a_sqhat=1,sigma_b_sqhat=1,
  epsilon,max_iter=100,over_dispersed=TRUE,trace.it=TRUE,epsilon_backfitting=1e-10,inner_trace.it=TRUE){
  #it estimates the parameter using modified schall algorithm
  iter = 0
  nf1 = length(unique(f1))
  nf2 = length(unique(f2))
  ahat = rep(0,nf1)#rnorm(nf1)
  bhat = rep(0,nf2)#rnorm(nf2)
  dispersion=1
  ...
  while((rel_change>epsilon) & (iter < max_iter)){
    ...
    cat("time taken to pre compute factor a matrix",difftime(Sys.time(),start_time_factor_a,units="secs"),"\n")

    start_time_factor_b = Sys.time()
    Xclub_b = preclub(x,f2,weight,lambda_b_hat)$operator
    cat("time taken to pre compute factor b matrix",difftime(Sys.time(),start_time_factor_b,units="secs"),"\n")

    scha = backfitting_inner_loop(x,z,f1,f2,weight,weight_a,weight_b,betahat,ahat,bhat,
      lambda_a_hat,lambda_b_hat,max_iter,epsilon_backfitting,inner_trace.it,Xclub_a,Xclub_b)
...
schall_covariance<-function(x,f1,f2,lambda_a_hat,lambda_b_hat,weight,
  ...
  while((rel_change>epsilon) & (iter < max_iter)){
    ...
    preda = a_hat[f1,]
    #I am keeping the weights and lambda_a_fixed at convergence of the logistic betahat
    #but when we obtianed the betahats, we had different lambdas and weight at each iterations
...
var_betahat_glmm <-function(x,f1,f2,trace.it=TRUE,backfit,epsilon=1e-10,max_iter=100){
  ...
  #xt_w_i_minus_s_x_inv%*%((1/weight_vector)*t(xt_w_i_minus_s_x_inv))
...
var_betahat_logistic_under_glmm <-function(x,f1,f2,trace.it=TRUE,backfit,cov,logistic_fit_w){
  ...
  # temp = sigma_a_sqhat*t(za_t_wx)%*%za_t_wx + sigma_b_sqhat*t(zb_t_wx)%*%zb_t_wx +
